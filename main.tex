...
\begin{figure}[h]
  \centering
  \includegraphics[width=1\textwidth]{./graphics/images/tts/dvt tts full workflow.png}
  \caption{data preprocessing workflow for tts}
...
First we tried a fully automated way for the dataset creation: In this case we input one or multiple audiofiles and start the process. Based on the tools we've just mentioned, the audio gets transcribed and marked with a speaker ID.
...
Now we had all clips cut out cleanly, so we uploaded them again into \gls{dvt} to finalize the dataset creating by performing one last transcription (with text normalization) to end up with a correct metadata.csv file (see figure \ref{fig:metadata.csv}). \\
...
%General
Compared to \gls{tts}, \gls{rvc} is pretty straight forward. Regarding \gls*{dvt}, almost no adaptations to the original \gls*{rvc} \cite*{RVCProjectRetrievalbasedVoiceConversionWebUI2023} code have been made. \gls*{dvt} mostly serves as a CUDA runtime environment to provide all drivers, dependencies and required AI models to \gls{rvc}. We added is the aforementioned \verb|filebrowser| to make file transfers to a remote GPU Server easier. Of cource, we can employ our preprocessing tools to aid the dataset generation process.\\
...
Once 10 minutes of snippets all under 10 seconds are in the RVC directory we can move on to training with \verb|rvc|. This will start up the Gradio ui which we need to access via our browser. From there we can start the training. The GUI, together with the online documentation is quite easy to understand and thus won't be explained further. This also counts for inferencing/converting voices with \gls{rvc}.
